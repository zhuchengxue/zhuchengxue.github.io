...
- 自动生成文章级 Open Graph 分享图、文章结构化数据与 `llms.txt`
...
它会检查首页、文章页、RSS、JSON Feed、OpenSearch、llms.txt、sitemap、Manifest、示例文章，以及线上 `/search.json` 是否包含 71 篇全文搜索索引。
...
脚本会检查 `dist/` 关键文件，包括首页、RSS、JSON Feed、OpenSearch、llms.txt、sitemap、Manifest 和搜索索引，把产物推送到镜像仓库指定分支，并自动写入 `.nojekyll`。dry-run 或加 `--report` 时会生成 `exports/mirror-report.json`，记录文件数量、体积、目标仓库和分支。如果镜像也绑定域名，可以额外设置：
