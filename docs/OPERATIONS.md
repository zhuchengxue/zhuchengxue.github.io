...
线上体检会同时检查首页、文章页、RSS、JSON Feed、OpenSearch、llms.txt、sitemap、Manifest、示例文章和 `/search.json`，确认全文搜索索引已发布且包含 71 篇文章。
...
镜像脚本会检查 `dist/` 关键文件，包括首页、RSS、JSON Feed、OpenSearch、llms.txt、sitemap、Manifest 和搜索索引，并在 dry-run 或加 `--report` 时生成 `exports/mirror-report.json`。它不会保存 token，也不会自动运行。
