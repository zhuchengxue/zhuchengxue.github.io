...
- 微信常见的 `#js_content` / `.rich_media_content` 正文容器会被单独提取，避免把页面导航和页脚混进文章
- `data-src` / `data-original` 懒加载图片会恢复为 Markdown 图片
- 优先从页面元数据恢复标题、发布日期、摘要和公众号原文地址
...
脚本会检查 `dist/` 关键文件，包括首页、RSS、JSON Feed、OpenSearch、llms.txt、humans.txt、sitemap、Manifest 和搜索索引，把产物推送到镜像仓库指定分支，并自动写入 `.nojekyll`。dry-run 或加 `--report` 时会生成 `exports/mirror-report.json`，记录文件数量、体积、目标仓库和分支。如果镜像也绑定域名，可以额外设置：
